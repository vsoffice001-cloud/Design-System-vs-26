    ...
    <div className="min-h-screen bg-white flex">
      <aside className="w-64 border-r border-black/8 bg-white sticky top-0 h-screen overflow-y-auto flex-shrink-0">
        ...
        <div className="px-4 py-4 border-t border-black/8 mt-8">
          <p className="text-xs text-black/40">Version 2.0.0</p>
          <p className="text-xs text-black/40 mt-1">Last updated: Jan 2026</p>
        </div>
      </aside>
    ...
    <div className="space-y-12">
      ...
      <section>
        <div className="max-w-3xl">
          <div className="mb-4 flex items-center gap-3">
            <span className="px-3 py-1 bg-[var(--accent-red)] text-white text-xs font-medium rounded-full">v2.0.0</span>
            <span className="text-xs text-black/40">Released January 28, 2026</span>
          </div>
          <h1 className="text-5xl font-normal mb-6">Welcome to the<br />Design System</h1>
          ...
          <div className="flex gap-4">
            <Button variant="primary" size="lg" icon={<Book size={20} />} onClick={() => onNavigate('overview', 'quickstart')}>
              Get Started
            </Button>
            <Button variant="secondary" size="lg" onClick={() => onNavigate('components', 'buttons')}>
              View Components
            </Button>
          </div>
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <FeatureCard icon={<Layers size={24} />} title="Atomic Design" description="Organized using atomic design methodology for maximum scalability" />
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="space-y-8">
        <PrincipleCard number="01" title="Minimalist Editorial" description="Clean, sophisticated, and content-focused design that prioritizes readability and user comprehension with ample breathing room." principles={['Black/white foundation with strategic Ken Bold Red (#b01f24) accent (5% usage - CTAs only)', 'Generous white space and warm off-white (#f5f2f1) sections for visual hierarchy', 'Typography-first approach with Noto Serif for headlines, DM Sans for body', 'Content-focused layouts that let the work speak for itself']} />
        <PrincipleCard number="02" title="Hierarchy Through Scale" description="Major Third typography scale (1.25 ratio) creates clear, mathematical visual hierarchy from 12.8px to 76px, ensuring perfect readability at every level." principles={['Base size: 16px with systematic scaling (xs → sm → base → lg → xl → 2xl → 3xl → 4xl → 5xl)', 'Typography scale ensures proportional relationships between all text sizes', 'Massive hero headings (48-76px) for editorial impact', 'Responsive type sizing with clamp() for fluid scaling across devices']} />
        <PrincipleCard number="03" title="Sophisticated Interactions" description="Purposeful motion serves clear functions—guiding attention, providing feedback, and enhancing spatial understanding without distraction." principles={['Four-tier duration system: Instant (100ms), Fast (200ms), Base (300ms), Slow (500ms)', 'Ease-out (cubic-bezier(0.22, 1, 0.36, 1)) for natural deceleration', 'Subtle micro-interactions for delight: hover states, gradient shifts, scale transforms', 'Respects prefers-reduced-motion for accessibility']} />
        <PrincipleCard number="04" title="Atomic Design Methodology" description="Systematic component organization from atoms to organisms ensures scalability, reusability, and predictable patterns that reduce cognitive load." principles={['Atoms: Buttons, inputs, icons, color swatches', 'Molecules: Form groups, card headers, navigation items', 'Organisms: Navigation bars, cards, sections with multiple components', 'Design tokens for all visual properties (colors, spacing, typography, shadows, border radius)']} />
        <PrincipleCard number="05" title="Accessible by Default" description="Every component meets WCAG 2.1 AA standards minimum, ensuring inclusive experiences for all users regardless of abilities or assistive technology." principles={['Minimum 4.5:1 contrast ratios for normal text, 3:1 for large text', 'Semantic HTML with proper heading hierarchy (h1 → h2 → h3)', 'Full keyboard navigation support with visible focus states', 'Screen reader optimization with ARIA labels and roles', 'Touch targets minimum 40px × 40px for mobile accessibility']} />
        <PrincipleCard number="06" title="Systematic Consistency" description="Predictable patterns and reusable components create familiarity across the system, making it intuitive for both designers and developers." principles={['All design tokens defined as CSS variables and TypeScript constants', 'Spacing scale based on 4px grid (0.25rem base unit)', 'Border radius system: small (6px), medium (10px), large (16px), xl (20px)', 'Comprehensive documentation with WHY/WHAT/WHEN/WHEN NOT/HOW for every component']} />
        <PrincipleCard number="07" title="Scalability & Flexibility" description="Components handle varying content volumes (2-10+ items) and adapt responsively across all device sizes from mobile (375px) to ultra-wide (1920px+)." principles={['Mobile-first responsive design with breakpoints: sm (640px), md (768px), lg (1024px), xl (1280px), 2xl (1536px)', 'Grid systems that adapt: 1 column mobile → 2-3 columns tablet → 3-4 columns desktop', 'Flexible components with content constraints (max-width: 1000px for optimal readability)', 'Components gracefully handle empty states, loading states, and error states']} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="space-y-6">
        <StepCardButton step="1" title="Understand the Foundations" description="Start by familiarizing yourself with our core design tokens: colors, typography, and spacing." action="View Foundations" onClick={() => onNavigate('foundations', 'colors')} />
        <StepCardButton step="2" title="Explore Components" description="Browse our comprehensive component library with live examples and code snippets." action="Browse Components" onClick={() => onNavigate('components', 'buttons')} />
        <StepCardButton step="3" title="Review Patterns" description="Learn common design patterns for layouts, navigation, and content organization." action="See Patterns" onClick={() => onNavigate('patterns', 'page-layouts')} />
...
function WhatsNewContent() {
  return (
    <div className="space-y-12">
      ...
      <div className="space-y-8">
        <ChangelogSection version="2.0.0" date="January 2026" type="major" changes={[{category: 'New', items: ['Added comprehensive Motion & Animation system with 4-layer duration scale', 'New Layout Patterns section with grid systems and z-index strategy', 'Stripe-style dashboard with side navigation', 'Interactive component playground']}, {category: 'Enhanced', items: ['Expanded typography section with responsive sizing examples', 'Added 6 new spacing helper components', 'Improved color accessibility documentation', 'Enhanced button component with more variants']}]} />
        <ChangelogSection version="1.0.0" date="December 2025" type="initial" changes={[{category: 'Initial Release', items: ['Complete design system with 7 categories', 'Atomic design methodology implementation', '50+ documented components', 'Comprehensive WHY/WHAT/WHERE/WHEN/HOW framework']}]} />
      </div>
    </div>
  );
}
  ...
  return (
    <button onClick={onClick} className="p-4 border border-black/8 rounded-lg hover:border-black/20 hover:bg-black/2 transition-all group">
      <div className="flex items-start justify-between">
        <div>
          <h3 className="font-medium mb-1 group-hover:text-[var(--accent-red)] transition-colors">{title}</h3>
          <p className="text-sm text-black/60">{description}</p>
        </div>
  ...
  return (
    <div className="flex gap-6 p-6 border border-black/8 rounded-lg hover:border-black/15 transition-colors">
      ...
      <div className="flex-1">
        ...
        <p className="text-black/70 mb-4">{description}</p>
        <button className="text-sm font-medium hover:text-[var(--accent-red)] transition-colors inline-flex items-center gap-2" onClick={onClick}>
          {action}
  ...
  return (
    <div className="border border-black/8 rounded-lg overflow-hidden">
      <div className="bg-black/5 px-4 py-2 border-b border-black/8 flex items-center justify-between">
        ...
        <button className="text-xs font-medium hover:text-[var(--accent-red)] transition-colors">Copy</button>
      </div>
      <pre className="p-4 overflow-x-auto bg-black/2"><code className="text-sm font-mono">{code}</code></pre>
    </div>
  );
}
...
function ChangelogSection({ version, date, type, changes }: { version: string; date: string; type: 'major' | 'minor' | 'initial'; changes: { category: string; items: string[] }[] }) {
  const typeColors = {major: 'bg-[var(--accent-red)] text-white', minor: 'bg-black/10 text-black', initial: 'bg-black text-white'};
